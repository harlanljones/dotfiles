...
- Wallpaper: ✅ Implemented as deterministic PNG gradient generation (blends `darker_background` → `background` blended 55% toward `accent`). New script `run_onchange_after_46-vespasian-wallpaper.sh.tmpl` applies via `SystemParametersInfo(SPI_SETDESKWALLPAPER)`. All 8 themes (4 legacy Tokyo Night + 4 Omarchy) have regenerated wallpapers.
