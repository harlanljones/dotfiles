...
Run only the discovery commands needed for the task. Login and config creation
are user-authorized setup, not automatic preflight mutations. Load credentials
using the entrypoint before each workflow; stop that workflow if loading fails.
...
Pass the resolved `--project` when the task has one; add
`--no-use-default-template` when the supplied body is the complete intended brief.
Inspect current create help before relying on these flags.
...
First verify acceptance evidence and whether the originating workflow requires
human review instead of completion. Preserve a partial-work handoff before
unassigning; making incomplete work reclaimable without context risks duplication.
...
The same limitation applies to assignment. Re-read before and after a claim,
but use a single dispatcher for shared-account workers rather than treating
those reads as a transactional lock.
