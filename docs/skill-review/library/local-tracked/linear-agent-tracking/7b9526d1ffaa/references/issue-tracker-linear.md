...
Use this as the starting content for a repository's `docs/agents/issue-tracker.md`. Replace bracketed values from observed configuration or user input; do not invent them.
...
- Command: `[linear or repo wrapper]`
- Workspace: `[workspace slug]`
- Team: `[team key]`
- Project: `[project name/id, or none]`
- Selection policy: lowest-number eligible open ticket; re-read current claims
  and native blockers before selecting work.
...
- Create: `linear issue create --no-interactive --team [TEAM] --title "..." --description-file <path>`
- Read: `linear issue view [ID] --json --no-download`
- Query: `linear issue query --team [TEAM] --all-states --all-assignees --json`
...
- Coordination: one dispatcher for shared-account workers. Assignment has no
  atomic compare-and-swap guarantee; record worker scope and re-read ownership.
...
For implementation work awaiting human approval, use the team's review state
or leave it open with evidence. A resolution state must mean the ticket's actual
acceptance criteria are met, not just that an agent stopped running.

## Pull requests as a triage surface

**PRs as a request surface: [yes/no].** Linear and the code host do not share a number space. Reference a Linear issue by its full identifier.
```
