...
description: |
  ...
  refiners qwen3.8-max-preview + DeepSeek V4 Pro + Codex Opus 5 high) before producing a final implementation
...
`qwen`/qwen3.8-max-preview + `deepseek`/deepseek-v4-pro + `opus`/Codex-opus-5 high)
...
- Tasks that fit in a single Codex turn
...
```
Layer 0 — Spec triage                      (parent Codex, in-place)
   │
   ├─ read spec
...
Layer 2 — Broadcast refiners               (3 parallel; each sees ALL valid proposals)
   ...
   │
   └─ opus refines the broadcast (Codex-opus-5 @ high)
...
Layer 0 happens in this Codex session. Layers 1 and 2 are spawned as
...
aggregator. Fable is an optional quota-heavy Codex aggregator only and is
...
```bash
python3 ~/.Codex/skills/mixture-of-agents/scripts/install_deps.py
```
This is config-aware: it checks that every harness your resolved roster needs
(for the default roster: AGY, Codex, Codex, and OpenCode) is installed and
...
Read `~/.Codex/skills/mixture-of-agents/prompts/scout.md` for the full
...
```bash
python3 ~/.Codex/skills/mixture-of-agents/scripts/run_moa.py \
...
8. **Read-only discipline is non-negotiable.** All proposers and refiners
   ...
   enforcement via `--sandbox read-only`; Codex gets a hard read-only tool
...
- `SKILL.md` (this file) — protocol Codex follows when invoked
...
- `scripts/adapters/Codex.py` — Codex CLI subprocess wrapper (sonnet proposer)
...
- **Repo grounded.** All CLIs run with read-only discipline (filesystem
  sandbox for Codex, tool allowlist for Codex, permission-deny policy for
