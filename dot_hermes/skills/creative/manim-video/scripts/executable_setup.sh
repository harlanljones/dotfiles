...
if python3 -c "import manim" 2>/dev/null; then
  ...
  fail "Manim not installed: pip install manim"; errors=$((errors+1))
fi
if command -v pdflatex &>/dev/null; then
  ok "LaTeX (pdflatex)"
else
  fail "LaTeX not found (macOS: brew install --cask mactex-no-gui)"; errors=$((errors+1))
fi
if command -v ffmpeg &>/dev/null; then
  ok "ffmpeg"
else
  fail "ffmpeg not found"; errors=$((errors+1))
fi
echo ""
if [ $errors -eq 0 ]; then
